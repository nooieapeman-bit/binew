...
# Injection Data
# Same logic, just modify Price Map for Basic
...
def calc_sub_avg_sim(desc, counts, prices_map):
    ...
    print(f"\n{desc} (Simulated):")
    print(f"  Monthly Avg: {m_avg:.2f} EUR (prev: vs Old)")
    print(f"  Yearly Avg:  {y_avg:.2f} EUR (prev: vs Old)")

calc_sub_avg_sim("Group A", counts_a, prices_sim)
calc_sub_avg_sim("Group B", counts_b, prices_sim)
